...
void initializationLoop()
{
	while (1)
	{
		...
		if (~PINA & (1 << 1)) // SW 2 lock in count down value
		{
			while (~PINA & (1 << 1))
			{
...
void downCountLoop()
{
	...
	while (1)
	{
		if (~PINA & (1 << 4)) // SW6 begin countdown
		{
			while (~PINA & (1 << 4))
			{
			} // wait for button release
			while (currentCount != 0)
			{
				delay(500); // wait .5s
				currentCount--;
				PORTD = flip(currentCount); // display current count on LEDs
			}
			CountingVar = 0; // reset initialization count
			PORTD = 0x00;	 // turn off all LEDs
			break;
		}
	}
	play_tone(1250, 750); // play 1.25kHz square wave (and flash LEDs once)
	return;
}
...
void play_tone(uint16_t frequency, uint16_t duration_ms) // play a given frequency for a given amount of time
{
	...
	for (uint32_t i = 0; i < total_toggles; i++)
	{
		...
		if (frequency == 1250) // if end of countdown (1.25kHz)
		{
			PORTD |= 1 << 0; // keep the first three pins off
			PORTD |= 1 << 1;
			PORTD |= 1 << 2;
			PORTD ^= 1 << 3; // toggle the last 5 bits
			PORTD ^= 1 << 4;
			PORTD ^= 1 << 5;
			PORTD ^= 1 << 6;
			PORTD ^= 1 << 7;
